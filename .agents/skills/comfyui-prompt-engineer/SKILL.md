...
## 教材專案客製化規則 (Project-Specific Rules)

> ⚠️ **注意**：根據本教材的受眾設定（美術設計單位），當您生成或提供上述提示詞與參數時，必須嚴格遵守以下溝通準則：
> 1. **術語翻譯**：向使用者解釋參數時，請將技術名詞轉化為設計師熟悉的語言。
>    - 例如：`CFG (Classifier Free Guidance)` ➔ 「靈感發散度」或「自由發揮度」
>    - 例如：`Target Checkpoint` ➔ 「基礎畫風引擎」
>    - 例如：`Negative Prompt` ➔ 「黑名單 / 違禁品列表」
> 2. **對話式引導**：提供提示詞時，不要只是丟出程式碼區塊，可以加入生動的引導，如：「老闆，我們換個『靈感發散度 (CFG)』為 4 的設定來看看，這樣 AI 會更聽您的話...」。
