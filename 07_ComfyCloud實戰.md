...
*(2.2 抽盲盒地獄結束)*
☕ **【中場休息 10 分鐘】抽盲盒抽到崩潰了嗎？先去喝口水，讓大腦冷卻一下！**
...
### 4.2 實作三：精準描圖
*(預計時間：25 分鐘)*
*   在畫布空白處點擊兩下左鍵開啟搜尋框 ➔ 輸入並加入 **【ControlNet Apply】** 與 **【Load Image】** 節點。
*   上傳一張有明確骨架或邊緣線的參考圖 ➔ 用滑鼠左鍵點住原圖的黃色圓點，拖曳連線到 ControlNet 的輸入端 ➔ 再次點擊 **【Queue Prompt】**。
...
*(4.2 實作三結束)*
☕ **【中場休息 10 分鐘】ControlNet 是個非常複雜的觀念，我們先讓大腦消化一下強制描圖的威力。喝口水，我們等等要進入最容易當機的地獄了！**
...
### 5.1 觀念四：模型也有「血型」或「主機平台」
* **Model Architecture Mismatch (模型架構不匹配)**：這是初學者最常遇到的地獄。AI 模型有分不同的世代架構 (例如：SD1.5, SDXL, FLUX)。這就像是你不能把 **PS5 的遊戲光碟 (SDXL LoRA)** 硬塞進 **PS4 的主機 (SD1.5 Checkpoint)** 裡一樣，硬塞只會當機報錯。
* **生活化比喻 (輸血理論)**：
...
* **生活化比喻 (影像格式)**：
  * **FP16 (豪華原畫質)**：就像是相機的 **RAW 檔**。保留了最完美的色彩與光影細節，但檔案超級肥大，會吃掉很多顯卡記憶體 (VRAM)。
  * **FP8 (輕量壓縮版)**：就像是高品質的 **JPG 檔**。檔案大小直接少一半，跑得更快，而且對設計師來說「肉眼幾乎看不出畫質差異」。
* **精度需要匹配嗎？ 完全不需要！**
...
* 點選錯誤的 VAE 節點並按下鍵盤的 Delete 鍵將其刪除，雙擊空白處搜尋並載入正確對應 FLUX 的 `ae.safetensors` VAE，再重新將黃點連線 ➔ 再次點擊 **【Queue Prompt】** 生成。
